...
it has loaded the live visible list and derived a real authenticated actor id, so a half-loaded
screen cannot bypass duplicate-name checks and an unauthenticated identity never creates a Drive
folder. If a folder exists without a matching foundation Project record, the browser marks it as
incomplete and retries initialization on that same folder instead of creating a second one; if
the saved annotations are corrupt or unreadable, the row surfaces an error and won't open. It's
strictly additive—set nothing and it doesn't exist. The security posture stays honest: still a
plain static site, **no secrets in the bundle**, team-only because the Google OAuth app is
**Internal** to your domain, and the data sits in **your own Drive**. See
